...
# 🔧 FIX: Ahora las A4 se imprimen desde el navegador del cliente
#         El backend solo registra en log (no toca SumatraPDF/Brother)
# ============================================================

@app.route('/generar_ficha_a4', methods=['POST', 'OPTIONS'])
def generar_ficha_a4():
    """
    🔧 FIX: Ahora solo registra en log.
    La impresión real la hace el navegador del cliente con window.print(),
    lo que permite imprimir en cualquier impresora A4 configurada en cada PC.
    """
    if request.method == 'OPTIONS':
        return '', 200
    try:
        ...
        lote = datos.get('lote', 'S/L')
        fecha = datos.get('fecha_prep', 'S/F')
        
        print(f"📄 Ficha A4 solicitada (impresión en cliente):")
        print(f"   → Preparación: {nombre}")
        print(f"   → Lote: {lote}")
        print(f"   → Fecha: {fecha}")
        
        return jsonify({
            "status": "success",
            "message": "Ficha registrada. El navegador gestionará la impresión.",
            "modo": "client-side",
            "ficha": {"nombre": nombre, "lote": lote, "fecha_prep": fecha}
        }), 200
    except Exception as e:
...
def generar_ficha_a4_masiva():
    """
    🔧 FIX: Igual que la individual, solo registra en log.
    El navegador imprime todas las fichas en una sola ventana.
    """
    if request.method == 'OPTIONS':
        return '', 200
    try:
        ...
        print(f"📄 {len(fichas)} fichas A4 solicitadas (impresión en cliente):")
        for i, f in enumerate(fichas[:5], 1):
            print(f"   {i}. {f.get('nombre', 'S/N')} | Lote: {f.get('lote', 'S/L')}")
        if len(fichas) > 5:
            print(f"   ... y {len(fichas) - 5} más")
        
        return jsonify({
            "status": "success",
            "message": f"{len(fichas)} fichas listas para imprimir en el navegador",
            "modo": "client-side",
            "total": len(fichas)
        }), 200
    except Exception as e:
...
# ============================================================
# 🖨️ STICKERS → SIGUEN USANDO LA BROTHER (SIN CAMBIOS)
# ============================================================
...
# ============================================================
# 📦 NUEVOS ENDPOINTS DE RECEPCIÓN (HISTÓRICO)
# ============================================================

@app.route('/obtener_recepciones', methods=['GET', 'OPTIONS'])
def obtener_recepciones():
    ...
    try:
        if supabase:
            result = supabase.table('recepciones').select("*").order('fecha_registro', desc=True).execute()
            return jsonify(result.data if result.data else []), 200
        return jsonify([]), 200
    except Exception as e:
        print(f"❌ Error en obtener_recepciones: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route('/guardar_recepcion', methods=['POST', 'OPTIONS'])
def guardar_recepcion():
    ...
    try:
        ...
        producto = datos.get('producto', '').upper()
        if not producto:
            return jsonify({"status": "error", "message": "Producto es obligatorio"}), 400
        
        ahora = datetime.now().isoformat()
        recepcion = {
            "id": datos.get('id', f"REC_{datetime.now().strftime('%Y%m%d%H%M%S')}_{producto[:6]}"),
            "producto": producto,
            "proveedor": datos.get('proveedor', 'S/P').upper(),
            "cantidad": int(datos.get('cantidad', 1)),
            "unidad": datos.get('unidad', 'UN').upper(),
            "fecha_registro": datos.get('fecha_registro', ahora),
            "fecha_recibido": datos.get('fecha_recibido', ahora),
            "observaciones": datos.get('observaciones', '')
        }
        
        if supabase:
            result = supabase.table('recepciones').insert(recepcion).execute()
            print(f"📦 Recepción guardada en Supabase: {producto}")
            
            try:
                channel = supabase.channel('recepciones')
                channel.send({
                    'type': 'broadcast',
                    'event': 'nueva_recepcion',
                    'payload': recepcion
                })
            except:
                pass
            
            return jsonify({"status": "success", "data": recepcion}), 200
        
        return jsonify({"status": "error", "message": "Supabase no disponible"}), 500
            
    except Exception as e:
        print(f"❌ Error en guardar_recepcion: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route('/eliminar_recepcion', methods=['POST', 'OPTIONS'])
def eliminar_recepcion():
    ...
    try:
        ...
        id_recepcion = datos.get('id')
        if not id_recepcion:
            return jsonify({"status": "error", "message": "ID es obligatorio"}), 400
        
        if supabase:
            supabase.table('recepciones').delete().eq('id', id_recepcion).execute()
            return jsonify({"status": "success"}), 200
        
        return jsonify({"status": "error", "message": "Supabase no disponible"}), 500
            
    except Exception as e:
        print(f"❌ Error en eliminar_recepcion: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
...
def health_check():
    ...
    return jsonify({
        ...
        "version": "5.7-ClientPrintA4",
        "supabase": "connected" if supabase else "disconnected",
        "database": conn_status,
        "threads": threading.active_count(),
        "pool_size": len(db_pool._connections),
        "impresora_etiquetas": "SumatraPDF (Brother QL-810W)",
        "impresora_a4": "Navegador del cliente (window.print)"
    }), 200
...
if __name__ == '__main__':
    ...
    print("🖨️ Modo de impresión:")
    print("   Etiquetas/Stickers → SumatraPDF → Brother QL-810W")
    print("   Fichas A4         → Navegador del cliente (window.print)")
    print("   IP Brother: 172.20.101.56")
    print("   Endpoints disponibles:")
    print("     - /imprimir_lote → Etiquetas completas (Brother)")
    print("     - /imprimir_etiqueta_rapida → Etiquetas rápidas (Brother)")
    print("     - /imprimir_etiqueta_rapida_v2 → Etiquetas rápidas ASÍNCRONO (Brother)")
    print("     - /generar_ficha_a4 → Registro A4 (impresión en cliente)")
    print("     - /generar_ficha_a4_masiva → Registro A4 masivo (impresión en cliente)")
    print("     - /generar_sticker → Sticker individual (Brother)")
    print("     - /generar_sticker_masivo → Stickers masivos (Brother)")
    print("     - /obtener_recepciones → Obtener historial de recepciones")
    print("     - /guardar_recepcion → Guardar nueva recepción en historial")
    print("     - /eliminar_recepcion → Eliminar recepción del historial")
